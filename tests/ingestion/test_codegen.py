...
def test_legacy_generate_package_writes_unvalidated_rust_candidate(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    cfg = {
        "retrieval": {"top_k": 2},
    ...
    assert manifest["status"] == "generated_unvalidated"
    assert "production_candidate_validated" not in (out / "VALIDATION_REPORT.md").read_text()
